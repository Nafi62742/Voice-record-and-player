...
public class SoundFile {
    ...
    protected ByteBuffer getDecodedData() {
        return mDecodedBytes;
    }

    protected void setDecodedData(ByteBuffer combinedData, int totalChannels) {

        mDecodedBytes=combinedData;
        mChannels=totalChannels;
    }
    ...
    public static SoundFile create(String fileName,
        ...
        Log.d(TAG, "create: File path check "+f.getAbsolutePath());
...
//    }
    ...
    private void ReadMultipleFiles(List<File> inputFiles) throws IOException, InvalidInputException {
        mDecodedBytes = ByteBuffer.allocate(1 << 20); // Initialize with a reasonable buffer size

        for (File inputFile : inputFiles) {
            // Read each input file and append the decoded samples to mDecodedBytes
            ReadFile(inputFile);
        }

        // At this point, mDecodedBytes should contain the combined audio data of all input files.
        mDecodedBytes.rewind();
        mDecodedBytes.order(ByteOrder.LITTLE_ENDIAN);
        mDecodedSamples = mDecodedBytes.asShortBuffer();

        // Calculate the total number of samples for the combined audio
        mNumSamples = mDecodedSamples.remaining() / (mChannels * 2); // One sample = 2 bytes

        // Now, you can use mDecodedSamples and mNumSamples as needed for further processing.
    }
    ...
    public void WriteFile(File outputFile, float startTime, float endTime)
        ...
        try {
            ...
            Log.e("Ringdroid 3", "Works");
            outputStream.close();
        } catch (IOException e) {

            Log.e("Ringdroid", "Failed to create the .m4a file."+e.getMessage());
            Log.e("Ringdroid", getStackTrace(e));

            Log.e("Ringdroid 2", "Failed "+ e.getMessage());
            Log.e("Ringdroid 2", getStackTrace(e));
